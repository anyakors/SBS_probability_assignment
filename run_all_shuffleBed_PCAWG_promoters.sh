...
if [ ${stage} -le 0 ] && [ ${stop_stage} -ge 0 ]; then
	...
	python calc_occupancy_random_shuffleBed_pad.py \
		...
		--feature data/genePromoters_hg19_noTop05high.bed \
		...
		--out out_proba_PCAWG_promoters1000_BRCA_noTop05high \
		--temp_folder temp2 \
		--pad 1000
...
if [ ${stage} -le 1 ] && [ ${stop_stage} -ge 1 ]; then
	...
	python run_stats_single_bootstrap_shuffleBed.py \
		--folder out_proba_PCAWG_promoters1000_BRCA_noTop05high \
		--activities data/valid_pcawg_published_activities_BRCA.txt \
		--p_cutoff 0.05 \
		--N_perm 1000 \
		--N_sampled 10000 
